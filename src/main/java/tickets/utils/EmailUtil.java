...
public class EmailUtil {
    private static final String FROM = "13182811957@163.com";//发件人的email
    private static final String PWD = "523523521";//发件人密码--邮箱密码
    ...
    public static UserPO activateMail(UserPO userPO) throws MessagingException, NoSuchAlgorithmException {
        //注册邮箱
        String to  = userPO.getEmail();
        //当前时间戳
        Long curTime = System.currentTimeMillis();
        //激活码--用于激活邮箱账号
        String token = to+curTime;
        userPO.setToken(Encrypt.md5(token));
        token = userPO.getToken();
        System.out.println(" token ------------> :" + token);
        //激活的有效时间
        Timestamp activateTime = new Timestamp(curTime + TIMELIMIT);
        //过期时间
        System.out.println(" activateTime ------------> :" + activateTime);
        userPO.setActivateTime(activateTime);
        //发送的邮箱内容
